...
class AirTableBase(Base):
    ...
    _airtable_access_token_encrypted: Mapped[str | None] = MappedColumn(
        "airtable_token", String, nullable=True
    )
    ...
    def airtable_access_token(self) -> str | None:
        ...
        if self._airtable_access_token_encrypted:
            return fernet.decrypt(
                self._airtable_access_token_encrypted.encode()
            ).decode()
        return None
    ...
    def airtable_access_token(self, value: str) -> None:
        ...
        if value:
            self._airtable_access_token_encrypted = fernet.encrypt(
                value.encode()
            ).decode()
        else:

...
class ApertureElementFrame(Base):
    ...
    name: Mapped[str] = MappedColumn(
        String(255), nullable=False, default="Unnamed Frame"
    )
    frame_type_id: Mapped[str] = MappedColumn(
        String, ForeignKey("aperture_frame_types.id")
    )

    # Relationships
    frame_type: Mapped[ApertureFrameType] = relationship(
        "ApertureFrameType", back_populates="element_frames"
    )

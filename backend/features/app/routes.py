...
def update_project_settings_route(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to update project settings for '{bt_number}'",
        )
...
@router.get(
    "/get-project-airtable-table-identifiers/{bt_number}",
    response_model=list[AirTableTableSchema],
)
def get_project_airtable_table_identifiers(
    ...
    try:
        return [
            AirTableTableSchema.model_validate(t)
...
@router.patch(
    "/update-airtable-tables-identifiers/{bt_number}",
    response_model=list[AirTableTableSchema],
)
def update_airtable_tables_identifiers(
    request: Request,
    bt_number: str,
    table_records: list[AirTableTableUpdateSchema],
    db: Session = Depends(get_db),
) -> list[AirTableTableSchema]:

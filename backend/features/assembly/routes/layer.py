...
def create_new_default_layer_on_assembly_route(
    ...
    logger.info(
        f"assembly/create_new_default_layer_on_assembly_route({assembly_id=}, order={request.order})"
    )

    try:
        assembly, layer = insert_default_layer_into_assembly(
            db, assembly_id, request.order
        )
...
def update_layer_thickness_route(
    ...
    logger.info(
        f"assembly/update_layer_thickness_route(layer_id={layer_id}, thickness_mm={request.thickness_mm})"
    )

...
async def add_layer(request: CreateLayerRequest, db: Session = Depends(get_db)) -> JSONResponse:
    ...
    layer = Layer(assembly_id=assembly.id, thickness_mm=request.thickness_mm, order=request.order)
...
async def update_layer_thickness(
    ...
    layer = db.query(Layer).filter_by(id=layer_id).first()
    if not layer:
        return JSONResponse(content={"message": f"Layer with ID {layer_id} not found."}, status_code=404)
...
async def delete_layer(layer_id: int, db: Session = Depends(get_db)) -> JSONResponse:
    ...
    if len(assembly_layers) <= 1:
        raise HTTPException(status_code=400, detail="Cannot delete the last layer in the assembly.")
    ...
    db.query(Layer).filter(Layer.assembly_id == layer.assembly_id, Layer.order > layer.order).update(
        {"order": Layer.order - 1}, synchronize_session="fetch"
    )
    ...
    return JSONResponse(content={"message": f"Layer {layer_id} deleted successfully."}, status_code=200)

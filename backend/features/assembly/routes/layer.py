...
async def add_layer_to_assembly_route(
    request: CreateLayerRequest, db: Session = Depends(get_db)
) -> JSONResponse:
    """Add a new Layer to an Assembly."""
    logger.info(
        f"add_layer_to_assembly_route(assembly_id={request.assembly_id}, order={request.order})"
    )

    assembly, layer = add_default_layer_to_assembly(
        db, request.assembly_id, request.order
    )
...
async def update_layer_thickness_route(
    ...
    logger.info(
        f"update_layer_thickness_route(layer_id={layer_id}, thickness_mm={request.thickness_mm})"
    )
    ...
    return JSONResponse(
        content={
            "message": f"Layer {layer.id} updated to thickness: {layer.thickness_mm}."
        },
        status_code=200,
    )

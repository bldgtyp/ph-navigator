...
from features.assembly.services.to_hbe_material_steel_stud import (
...
def convert_assemblies_to_hbe_constructions(
    assemblies: list[Assembly],
) -> list[OpaqueConstruction]:
    """Convert a list of assemblies to Honeybee JSON format."""
    logger.info(
        f"convert_assemblies_to_hbe_constructions([{len(assemblies)}] assemblies)"
    )

    constructions_ = []
    for assembly in assemblies:
        if assembly.is_steel_stud_assembly:
            materials = get_steel_stud_layers_as_hb_materials(
                assembly.layers_outside_to_inside
            )
        else:
            materials = [
...
def get_all_project_assemblies_as_hbjson_string(db: Session, bt_number: str) -> str:
    ...
    return json.dumps(
        {hb_const.identifier: hb_const.to_dict() for hb_const in hbe_constructions}
    )

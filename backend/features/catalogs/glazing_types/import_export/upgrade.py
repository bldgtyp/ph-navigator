...
the destructive reshape in Alembic 20260604_0016.

v1→v2 (window-glass-catalog-enums) folds the legacy/typo values the AirTable
...
also **drop** a row entirely (the `DEFAULT` artifact rows), signalled by
returning `None`.
"""
...
from features.catalogs._option_seeds import (
    GLAZING_TYPE_DROP_MANUFACTURERS,
    GLAZING_TYPE_VALUE_FOLDS,
)
...
def _upgrade_v1_to_v2(row: dict[str, object]) -> dict[str, object] | None:
    ...
    Simpler than frame's: glazing has no swapped-field special case, only the
    casing fold (INTUS→Intus, ZOLA→Zola) and the DEFAULT-row drop.
    """
    upgraded = dict(row)

    # Drop the `DEFAULT` artifact rows entirely — the real default is the
    # recPHNDefGlazng01 sentinel (resolved by id, Phase 3).
    manufacturer = upgraded.get("manufacturer")
    if isinstance(manufacturer, str) and manufacturer.strip() in GLAZING_TYPE_DROP_MANUFACTURERS:
        return None

    # Value-level folds, keyed field → lower(trimmed) → canonical (casing only).
    for field, fold_map in GLAZING_TYPE_VALUE_FOLDS.items():
...
def upgrade_row(row: dict[str, object], from_version: int) -> dict[str, object] | None:
    ...
    drops the row (e.g. a `DEFAULT` artifact)."""

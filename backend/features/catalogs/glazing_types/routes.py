...
def post_glazing_type(
    payload: CatalogGlazingTypeCreateRequest, request: Request, auth: CatalogEditor
) -> CatalogGlazingTypePublic:
...
def delete_glazing_type_route(record_id: str, request: Request, auth: CatalogEditor) -> None:
    user, _expires_at = auth
    deactivate_glazing_type(record_id, user, request)


@router.post("/{record_id}/reactivate", response_model=CatalogGlazingTypePublic)
def reactivate_glazing_type_route(record_id: str, request: Request, auth: CatalogEditor) -> CatalogGlazingTypePublic:
    user, _expires_at = auth

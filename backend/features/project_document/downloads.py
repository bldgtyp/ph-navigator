...
def table_download_body(version_id: UUID, table_name: str, access: ProjectAccess) -> dict[str, object]:
    ...
    For custom-field-capable tables (Rooms, Thermal Bridges, Aperture
    Install Types, …) the value is the `{field_defs, rows}` envelope;
    bare-row tables (apertures) emit a list. Callers and tests must
    accept both shapes.
    """

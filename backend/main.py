...
JSON_WEB_TOKEN_SECRET_KEY = str(os.getenv("JSON_WEB_TOKEN_SECRET_KEY")) # to generate: openssl rand -hex 32
...
async def authenticate_user(db: Session, username: str, password: str) -> models.User | Literal[False]:
...
async def create_access_token(data: dict, expires_delta: timedelta | None = None) -> str:
    ...
    encoded_jwt = jwt.encode(to_encode, JSON_WEB_TOKEN_SECRET_KEY, algorithm=JSON_WEB_TOKEN_ALGORITHM)
    return encoded_jwt
...
async def get_current_user(token: Annotated[str, Depends(oauth2_scheme)], db: Session = Depends(get_db)) -> models.User:
    ...
    try:
        payload = jwt.decode(token, JSON_WEB_TOKEN_SECRET_KEY, algorithms=[JSON_WEB_TOKEN_ALGORITHM])

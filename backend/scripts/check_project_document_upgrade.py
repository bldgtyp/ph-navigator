...
AuditFailureCode = Literal["schema_version_too_new", "migration_error", "validation_error", "body_too_large"]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = audit_inputs(inputs, preview_dir=args.preview_dir, strict=args.strict)
...
def audit_inputs(inputs: Iterable[AuditInput], *, preview_dir: Path | None = None, strict: bool = False) -> AuditReport:
    records = [_audit_one(item, preview_dir=preview_dir) for item in inputs]
    schema_versions: dict[str, int] = {}
    upgrade_steps: dict[str, int] = {}
...
def _audit_one(item: AuditInput, *, preview_dir: Path | None) -> AuditRecord:

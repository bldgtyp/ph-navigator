...
Reads the committed `features/catalogs/glazing_types/seeds/glazing-types.v1.json`
envelope and pipes it through the same preview → commit pipeline the HTTP
routes use. Idempotent on `id` (rows already present in the catalog with the
same id are skipped); fresh ids are minted for seed rows that have none, so
the FIRST run is the realistic case and subsequent runs insert duplicates
unless the user re-exports the live catalog into the seed file.
"""

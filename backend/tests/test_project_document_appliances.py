...
from features.project_document.document import CURRENT_PROJECT_DOCUMENT_SCHEMA_VERSION, ApplianceRow, ProjectDocumentV1
...
def appliance_options() -> dict[str, list[dict[str, object]]]:
    return {
        ...
        "appliances.status": status_options_payload(),
    }
...
def test_legacy_equipment_appliances_contract_is_not_registered() -> None:
    with pytest.raises(HTTPException) as exc_info:
        get_table_contract("equipment_appliances")
    assert exc_info.value.status_code == 404
    detail = cast(dict[str, object], exc_info.value.detail)
    assert detail["error_code"] == "document_table_not_found"


def test_appliances_slice_exposes_status_field_and_options(clean_document_tables: None) -> None:
    client = signed_in_client()
    project = create_project(client)
    initial = client.get(draft_appliances_url(project["id"], project["active_version_id"]))

    assert initial.status_code == 200
    body = initial.json()
    assert_status_field_def(body["field_defs"])
    assert_status_options(body["single_select_options"], "appliances")


def test_appliances_replace_persists_status_value(clean_document_tables: None) -> None:
    client = signed_in_client()
    project = create_project(client)
    project_id = project["id"]
    version_id = project["active_version_id"]
    ...
    initial = client.get(draft_appliances_url(project_id, version_id))
    payload = appliance_payload()
    payload["appliances"][0]["custom_values"]["status"] = "opt_status_question"
    ...
    refetch = client.get(draft_appliances_url(project_id, version_id))
    assert refetch.json()["appliances"][0]["custom_values"]["status"] == "opt_status_question"
    assert_status_options(refetch.json()["single_select_options"], "appliances")


def test_appliances_replace_rejects_unknown_status_option(clean_document_tables: None) -> None:
    client = signed_in_client()
    project = create_project(client)
    project_id = project["id"]
    version_id = project["active_version_id"]
    ...
    initial = client.get(draft_appliances_url(project_id, version_id))
    payload = appliance_payload()
    payload["appliances"][0]["custom_values"]["status"] = "opt_status_bogus"

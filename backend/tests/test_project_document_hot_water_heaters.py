...
def hot_water_heater_payload() -> dict[str, Any]:
    return {
        ...
        "single_select_options": {
            ...
            "hot_water_heaters.status": status_options_payload(),
        },
    }
...
def test_hot_water_heaters_slice_exposes_status_field_and_options(clean_document_tables: None) -> None:
    client = signed_in_client()
    project = create_project(client)
    initial = client.get(draft_hot_water_heaters_url(project["id"], project["active_version_id"]))

    assert initial.status_code == 200
    body = initial.json()
    assert_status_field_def(body["field_defs"])
    assert_status_options(body["single_select_options"], "hot_water_heaters")


def test_hot_water_heaters_replace_persists_status_value(clean_document_tables: None) -> None:
    client = signed_in_client()
    project = create_project(client)
    project_id = project["id"]
    version_id = project["active_version_id"]
    ...
    initial = client.get(draft_hot_water_heaters_url(project_id, version_id))
    payload = hot_water_heater_payload()
    payload["hot_water_heaters"][0]["custom_values"]["status"] = "opt_status_question"
    ...
    refetch = client.get(draft_hot_water_heaters_url(project_id, version_id))
    assert refetch.json()["hot_water_heaters"][0]["custom_values"]["status"] == "opt_status_question"
    assert_status_options(refetch.json()["single_select_options"], "hot_water_heaters")


def test_hot_water_heaters_replace_rejects_unknown_status_option(clean_document_tables: None) -> None:
    client = signed_in_client()
    project = create_project(client)
    project_id = project["id"]
    version_id = project["active_version_id"]
    ...
    initial = client.get(draft_hot_water_heaters_url(project_id, version_id))
    payload = hot_water_heater_payload()
    payload["hot_water_heaters"][0]["custom_values"]["status"] = "opt_status_bogus"

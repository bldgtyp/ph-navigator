...
- Row overflow → modal *"Clipboard has N more rows. Add N empty
  records and paste?"*. Confirm appends N rows with final stable
  ULID-style ids generated by the frontend using the table prefix
  (`rm_...`, `erv_...`, `fan_...`, etc.); Cancel drops overflow.
  Column overflow drops silently with a toast.
...
type DataTableProps<TRow> = {
  ...
  // Locked mode — read-only (locked version or Viewer)
  readOnly?: boolean;
...
- `onWrite` is a single channel. Inline edits, paste, fill, undo, row
  ops all funnel here. The parent translates to project draft writes.
- Selection/focus/range state is **internal** to `<DataTable>` and
  ...
  need to read it.)
- `onWrite` is optimistic and queue-backed. The table applies one
  semantic `WriteOp` locally, then the parent persists it. For one
  table/draft, writes are FIFO and not sent concurrently.
- The table stays editable while writes are pending. The parent exposes
  save status ("syncing" / "saved" / "conflict") through the surrounding
  project shell; Save / Save As flush pending table writes before
  calling the save endpoint.
- On 409, auth failure, locked-version failure, or backend validation
  failure, the queue stops. The table rolls back to the last
  server-acknowledged snapshot, clears undo, and lets the parent show
  the conflict/session/locked/validation UI.
- `onViewChange` is not a document write and never marks the draft
  dirty. Chatty gestures are debounced or committed at gesture end:
  column resize commits on drag end, filter text debounces around
  250 ms, sort/group/hide changes emit immediately.
...
type FieldCapabilities<T> = {
  ...
  coerceFromClipboard: (raw: string, field: FieldDef) => CoerceResult<T>;
...
Clipboard coercion returns a structured result, not `T | null`.
Blank paste values become `{ ok: true, value: null }` only for nullable
fields. Required blanks, bad numbers, invalid units, failed custom
parsers, and invalid option labels return `{ ok: false, error, raw }`.
Multi-cell paste preflights the whole range before any draft write. If
any cell fails, no partial paste is committed; the UI shows a review
dialog with row / column / raw value / error, capped to the first 25
errors plus an "and N more" summary.
...
| `computed` | Read-only; renderer returns backend-provided overlay value. No editor. Computed in backend (CLAUDE.md: calculations live in backend). |
...
Decision confirmed 2026-05-11:

- Project document rows store user inputs, not default computed output.
- Computed values are backend-owned read overlays, returned alongside
  row data by table/domain endpoints:
  ```ts
  type ComputedOverlay = Record<
    string /* rowId */,
    Record<string /* fieldKey */, ComputedValue>
  >;

  type ComputedValue = {
    value: unknown;
    unit?: string;
    status: 'ready' | 'stale' | 'loading' | 'error';
    error?: string;
  };
  ```
- `computed` `FieldDef` entries declare a `compute_key`, dependency
  paths, and display/unit formatting. The parent uses dependencies to
  mark values stale and refetch overlays after successful draft writes.
- The frontend never reimplements domain calculations such as R-/U-value,
  ACH50, iCFA, or thermal-bridge totals. It renders returned values,
  loading/stale/error states, and tooltips.
- Computed overlay failure renders an explicit "Unable to compute" state
  or field-specific warning. Save is not blocked unless backend
  validation rejects the underlying inputs.

#### `single_select` lifecycle

Decision confirmed 2026-05-11:

- Row cells store stable `option_id` values, never labels.
- Rename changes only the option label; referenced rows display the new
  label without row rewrites.
- Reorder changes only `option.order`; sort follows option order.
- Duplicate labels are rejected within the same column after trim +
  case-insensitive comparison. Paste match-or-create matches existing
  labels instead of creating duplicates.
- Delete of an unreferenced option removes it from the option list.
- Delete of a referenced nullable option requires confirmation and then
  clears those cells to `null`.
- Delete of a referenced required option is blocked until the user
  reassigns affected rows or uses Merge.
- Merge rewrites all source option references to the target option and
  removes the source option records as one semantic write op.
- If a row references a missing option id, render a warning pill and
  block Save until the value is cleared, reassigned, or repaired by
  import tooling.
...
parent's `onWrite` for persistence. The parent translates to the
project draft JSON-Patch / table-replace endpoint.

Write semantics confirmed 2026-05-11:

- One table instance maintains a FIFO persistence queue per open
  project draft. It does not issue concurrent draft writes.
- The UI remains editable while the queue is flushing; pending state is
  reflected in the project save indicator.
- Save / Save As first wait for the table write queue to flush, then
  call the version save endpoint. If queue flush fails, Save / Save As
  does not proceed.
- On conflict, auth failure, locked-version failure, or backend
  validation failure, queued writes stop. The table rolls back to the
  last server-acknowledged snapshot, clears undo, and yields to the
  parent conflict/session/locked/validation UI.
- V1 does not attempt field-level merge after a failed async write.
- Browser-created document rows do not use temporary ids in v1. The
  frontend generates final stable ULID-style ids before the optimistic
  row appears. The backend validates prefix, shape, and uniqueness
  within the document table, then preserves the id. This keeps focus,
  selection, undo, and pending JSON-Patch paths stable.
...
These are reference-only examples: rewrite the patterns for V2 rather
than importing from `research/`.

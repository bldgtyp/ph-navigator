...
| Segmented control | `SegmentedControl` / `.phn-segmented-control*` | mutually exclusive inline single-select; native radio semantics; `xs`/`sm` compact chrome and `md` content pills. The small equal-width variant owns its app-wide hover wash and delayed option tooltip; tooltip copy falls back to `ariaLabel ?? label`, while other variants require an explicit option `tooltip` |
...
| Menus | `AppMenu` / `AppMenuItem`; `.app-menu*` / `.account-menu*` / `.catalog-menu*` | dropdown menus. Two variants beyond the icon-only default, both for a menu that **is** the action rather than an overflow affordance: `triggerLabel` renders a labelled trigger ("Set spec. status ▾"), and an item's `leading` fills the icon cell with something that is not a Lucide glyph (a status dot, a swatch) so its label still aligns with every other menu's |
...
| Checkbox | `.phn-check` | the app's square check mark — one paint recipe for every grid's select box (the DataTable row gutter, the report tables' select column). The consumer owns placement and reveal; this owns border/fill/tick/focus |
| Report selection + bulk action | `BulkStatusAction` (`shared/ui/report-table`) | the "N selected · Select all · Set spec. status ▾ · Clear" control, rendered in `StatusFilterChips`' summary slot while rows are selected. Selection state is the **page's**, not the table's: one page renders several report tables (in-scope / N/A / unused) that share one selection, so select-all lives here and `ReportTable` only takes `selectedRowIds` + `onToggleRowSelection` |
| Report table | `shared/ui/report-table` (`report-status-chip`) | status-dot / status-chip report grids. Optional row selection adds a frozen select lane ahead of the expand gutter (`report-table--selectable`), with the selected row taking the DataTable's `--data-table-selected-bg` wash; the table owns no selection state — see the Report selection row above. Column **headers use the DataTable's recipe** (`--font-mono`, `--fs-xs`, semibold, uppercase + caps tracking) — one spelling of "table header" app-wide; body cells are `--data-table-font-size` at regular weight, the primary column included. The table is its own horizontal scrollport, and freezes the expand gutter + primary column so a sideways-scrolled row keeps its identity — never wrap it in an outer `overflow-x` div, which cannot help because the clipping happens inside. An expanded row and its immediately following expansion share the split inset accent frame; nested report tables remain non-sticky |
| Hover tooltip | `<Tooltip>` (`shared/ui/tooltip`) | the standard hover/focus hint on a control — dark bubble, `TOOLTIP_HOVER_DELAY.medium/long`, `placement`. Prefer it over a native `title`. Tooltip uses a Popover anchor, not a trigger, so native controls retain their own ARIA and keyboard behavior. A **disabled** button fires no pointer events, so wrap it in an inline-flex span, put the tooltip on the span, and set `pointer-events: none` on the disabled button — otherwise the state that most needs explaining ("why is this greyed out?") is the one with no hint |

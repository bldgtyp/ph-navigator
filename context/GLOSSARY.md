...
| **Empty panel (void element)** | An Aperture Element with wire `kind: "void"` that tiles grid cells but represents host wall, not part of the aperture; excluded from U-value math, specification reports, and all exports. Its Glazed boundaries are window-to-wall junctions (jamb, sill, or head), not mullions. **Empty** is the UI label. A spandrel panel is not Empty; until `kind: "solid"` exists, represent it with a g=0 glazing entry. Distinct from an empty-state UI placeholder such as `ApertureEmptyState.tsx`. | Empty state, spandrel panel, blank cell |

...
| **Empty panel (void element)** | An Aperture Element with wire `kind: "void"` that tiles grid cells but represents host wall, not part of the aperture; it produces no calculation/specification element row and does not enter U-w or SHGC. Reports/exports may retain only its excluded-count metadata for reconciliation. Its Glazed boundaries are window-to-wall junctions (jamb, sill, or head), not mullions. **Empty** is the UI label. A spandrel panel is not Empty; until `kind: "solid"` exists, represent it with a g=0 glazing entry. Distinct from an empty-state UI placeholder such as `ApertureEmptyState.tsx`. | Empty state, spandrel panel, blank cell |
...
| **Install Type**      | A project-library window-install record (`aperture_install_types` row): a named Ψ-install value with source and optional Flixo/THERM report PDF; every project seeds an undeletable program-aware **Default** (Phius 0.052 / PHI 0.04 W/m·K) | Installation type, psi-install type |
| **Ψ-install (psi-install)** | The linear installation-junction heat-loss coefficient (W/m·K) of a window-to-wall edge; resolved per glazed-element side (assigned type → inherited Default), displayed alongside but always **excluded from** uninstalled U-w | Installation psi, psi-installation |
| **Perimeter vs interior (mulled) edge** | Per glazed-element side classification: a side whose neighboring cell is glazed is **interior (mulled)** and carries derived Ψ = 0 (never assignable); sides at the aperture boundary or against Empty panels are **perimeter** window-to-wall junctions and resolve to an Install Type | Mullion edge (the frame product is separate) |
...
| **Uninstalled U-w** | ISO 10077-1 whole-window thermal transmittance from glazing, frame, and glazing-edge terms before installation-junction losses; PHN displays Ψ-install separately and excludes it from this rollup | Installed U-value, U-window (when installation is included) |
| **45° corner split** | PHN's aperture frame-area convention: each rectangular frame corner is bisected diagonally, so each adjoining edge receives half of the shared corner area | Full-corner allocation |
| **Glazing-area-weighted SHGC** | Aperture SHGC calculated among elements with a numeric SHGC, weighted by glazing area only. Frame area and Empty panels do not enter the denominator; a missing glazing g-value warns and excludes that element from both numerator and denominator. | Whole-unit SHGC (unless the weighting basis is stated) |
...
| **Tab**             | One section of the Project workspace: Status, Climate, Apertures, Envelope, Spaces, Equipment, Thermal Bridges, Model, Documentation | Page, panel, view (use only contextually)         |
| **Documentation tab** | The project-wide evidence tab at `/projects/{id}/documentation`; summarizes specification status, datasheets, and site-photo coverage from the owning document tables for editors and read-only public viewers | Site Photos tab, evidence page                    |
...
| **Specification Status** | The shared per-record evidence status used by documentation-bearing tables (`Needed`, `Question`, `Complete`, `N/A`); stored on table rows but projected into the Documentation tab's spec axis | Spec status, product status                       |
| **Documentation axes** | The three independent completeness dimensions in the Documentation tab: specification status, datasheets, and site photos | Evidence axes, rollup axes                        |
| **Waiver ("not required")** | An explicit per-axis declaration that a datasheet or photo is not required for a row; counts that axis as complete without adding an attachment | N/A checkbox, exemption                           |

...
| Lessons | Local P1-13 hardening traced the active-token staging MCP `421 Invalid Host header` to FastMCP DNS-rebinding host validation after bearer auth. PHN now configures MCP transport security explicitly from `MCP_ISSUER_URL`, `MCP_RESOURCE_SERVER_URL`, `CORS_ORIGINS` for origin validation, and optional `MCP_ALLOWED_HOSTS` / `MCP_ALLOWED_ORIGINS`; final Phase 1 acceptance still requires redeploying those env values and passing the staging smoke in `docs/plans/2026-05-14/phase-1-release-gate.md`. |
...
| P1 | P1-13 | In progress; local gate passed, staging redeploy/HITL pending | 2026-05-14 | MCP transport-security hardening landed for the active-token staging `421 Invalid Host header` blocker: backend settings now expose `MCP_ENABLE_DNS_REBINDING_PROTECTION`, `MCP_ALLOWED_HOSTS`, and `MCP_ALLOWED_ORIGINS`; FastMCP receives explicit DNS-rebinding host allowlists derived from MCP URLs plus local defaults, with origin allowlists derived from MCP URLs/CORS. `.env.example` and `context/ENVIRONMENT.md` document the Render values. Requirements/evidence matrix recorded in `docs/plans/2026-05-14/phase-1-release-gate.md`. Local verification passed: `make db-up`; `make migrate`; `cd backend && uv run pytest tests/test_mcp.py -q` (6 passed); `make lint`; `make typecheck`; `git diff --check`; `make test` (backend 64 passed, frontend 62 passed); `cd frontend && npm run format:check`; `cd frontend && npm run build`; `make seed-dev-user`; `make e2e` (2 passed). Final Phase 1 acceptance still requires redeploying staging with the MCP allowlist env values and running the full staging smoke/HITL checklist. |

...
| Status | [ ] In progress; local gate passed, staging redeploy/HITL pending |
...
| Lessons | Local P1-13 hardening traced the active-token staging MCP `421 Invalid Host header` to FastMCP DNS-rebinding host validation after bearer auth. PHN now configures MCP transport security explicitly from `MCP_ISSUER_URL`, `MCP_RESOURCE_SERVER_URL`, `CORS_ORIGINS` for origin validation, and optional `MCP_ALLOWED_HOSTS` / `MCP_ALLOWED_ORIGINS`; after the first redeploy, staging health/session, CLI browser e2e, and Settings issue/revoke/Viewer-hidden checks passed, but active-token MCP still returned `421`, so derived deployed hosts now also include wildcard-port variants. Final Phase 1 acceptance still requires redeploying this follow-up and passing the staging MCP read smoke in `docs/plans/2026-05-14/phase-1-release-gate.md`. |
...
| P1 | P1-13 | In progress; staging MCP read redeploy pending | 2026-05-14 | MCP transport-security hardening landed for the active-token staging `421 Invalid Host header` blocker: backend settings now expose `MCP_ENABLE_DNS_REBINDING_PROTECTION`, `MCP_ALLOWED_HOSTS`, and `MCP_ALLOWED_ORIGINS`; FastMCP receives explicit DNS-rebinding host allowlists derived from MCP URLs plus local defaults, with origin allowlists derived from MCP URLs/CORS. First redeploy evidence: staging health `200` (request id `44000b0d-3cb7-48b4-a811-613c69d0a572`), unauthenticated session structured `401 not_authenticated` (request id `fb95c3e1-891d-4f2e-ac5f-97c4ce7a9667`), staging CLI e2e passed (2 Chromium tests), and browser Settings smoke patched metadata, issued an active MCP token, revoked a second token, and confirmed Viewer has no Project Settings button. Active-token MCP read still returned `421`, likely from a default-port Host header, so derived deployed hosts now include wildcard-port variants. Requirements/evidence matrix recorded in `docs/plans/2026-05-14/phase-1-release-gate.md`. Local verification remains passed: `make db-up`; `make migrate`; `cd backend && uv run pytest tests/test_mcp.py -q` (6 passed); `make lint`; `make typecheck`; `git diff --check`; `make test` (backend 64 passed, frontend 62 passed); `cd frontend && npm run format:check`; `cd frontend && npm run build`; `make seed-dev-user`; `make e2e` (2 passed). Final Phase 1 acceptance requires redeploying this wildcard-port follow-up and passing the staging active-token MCP read smoke. |

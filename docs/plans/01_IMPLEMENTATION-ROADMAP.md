...
| Status | [~] Shared extraction started; Rooms migrated to first shared read/copy/sort surface |
...
| P1 | P1-08 | In progress | 2026-05-13 | Shared `frontend/src/shared/ui/data-table/` extraction started and Rooms now renders through `<DataTable>` instead of `TablePrimitiveStub`; review/simplify cleanup fixed immediate paste/a11y/filtered-empty issues and tightened table helper reuse/hot paths. Verified with `cd frontend && npm run format:check`; `cd frontend && npm run lint`; `cd frontend && npm test` (44 passed); `cd frontend && npm run build`. Browser/POC parity checks remain before closing the slice. |

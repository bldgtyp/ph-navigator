...
| 6 | Active implementation | Started on `codex/assembly-builder-phase-06` from the Phase 5 branch tip. Specifications datasheet/photo evidence UI is wired to generic asset attach/detach routes, envelope attachment tests are added, and the attachment resolver now supports table-envelope rows such as Pumps. Browser smoke, Save As immutability workflow, and destructive photo-count dialogs remain. |

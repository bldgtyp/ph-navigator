...
export function ImportDialog({ onClose, onCommitted }: ImportDialogProps) {
  ...
  return (
    <ModalDialog title="Import frame-types JSON" titleId={titleId} onClose={resetAndClose}>
      {errorBanner ? (
      ...
      {stage.kind === "pick" ? (
        <div className="import-dialog-pick">
          <p>Select a Window-Frame Elements Catalog JSON file exported from PH-Navigator.</p>
          <input
            ref={fileInputRef}
            type="file"

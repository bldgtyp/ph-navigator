...
// approach this size (a 10k-row file is < 1 MB), so client-side
// rejection is purely a "don't bother POSTing it" optimization.
const IMPORT_MAX_BYTES = 8 * 1024 * 1024;

// Number of preview rows we render in the report. Backend already
// caps `rows_preview` to 50; we trim further for readability.
...
export type ImportDialogProps = {
  // Called when the dialog is dismissed. The component must be mounted
  // conditionally (e.g. `{open ? <ImportDialog ... /> : null}`); closing
  // tears down all internal state and any in-flight mutations. Avoids a
  // race where a late preview resolution leaves stale state visible on
  // the next open.
  onClose: () => void;
  // Fires after a successful commit (after the materials query is
  // invalidated by `useCommitImportMutation`). Page can use it to show
  // a toast or set focus.
...
export function ImportDialog({ onClose, onCommitted }: ImportDialogProps) {
  ...
  // Monotonic id per file pick. Late-resolving preview responses from
  // an earlier pick are discarded so the user can't end up looking at
  // file B's name with file A's report + token.
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    // Stamp this pick so any earlier in-flight pick that resolves
    // after us can detect it's stale and bail without setting state.
    ...
    let parsed: unknown;
    try {
      // Use FileReader rather than `file.text()` — the jsdom File
      // polyfill omits the text() method.
  ...
  return (
    <ModalDialog title="Import materials JSON" titleId={titleId} onClose={resetAndClose}>
      {errorBanner ? (
      ...
      {stage.kind === "pick" ? (
        <div className="import-dialog-pick">
          <p>Select a Materials Catalog JSON file exported from PH-Navigator.</p>
          <input
            ref={fileInputRef}
            type="file"
      ...
      {stage.kind === "report" ? (
        <ImportReport
          ...
          // Belt-and-braces: the 'committing' stage hides the
          // confirm button entirely, but if the user double-clicks
          // fast enough to land both clicks before React commits the
          // state transition, `commitMutation.isPending` blocks the
          // second call so we never POST /commit twice with the
          // same token.

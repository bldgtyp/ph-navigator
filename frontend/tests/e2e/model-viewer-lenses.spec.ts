...
test("switches model lenses, selects lens objects, and honors lens deep links", async ({ page }) => {
  ...
  await selectAnyModelObject(page, "ductSegmentLine");
  await expect(page.getByLabel("Selected model element")).toContainText("Duct Type");
  await expect(page.getByLabel("Selected model element")).toContainText(/Supply|Exhaust/);

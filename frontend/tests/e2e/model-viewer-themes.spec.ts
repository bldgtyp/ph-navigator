...
test("switches model themes, shows legends, and honors theme deep links", async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => window.__phnModelViewer?.legend?.rows)).toEqual([
  ...
  await expect.poll(() => page.evaluate(() => window.__phnModelViewer?.selectionId)).toBe(selectedFace);
  await expect
    .poll(() =>
      page.evaluate((objectId) => window.__phnModelViewer?.themeColorForObject(objectId), selectedFace),
    )
    .toMatch(/^#[0-9a-f]{6}$/);
  ...
  await expect.poll(() => page.evaluate(() => window.__phnModelViewer?.theme)).toBe("weighting-factor");
  await expect.poll(() => page.evaluate(() => window.__phnModelViewer?.legend?.rows)).toEqual([
    { id: "FullyTreated", label: "Fully Treated", color: "#F5E470", count: 5 },
  ]);
  ...
  await expect(page.locator(".model-legend-rows")).toBeHidden();

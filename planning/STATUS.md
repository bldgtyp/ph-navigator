...
| DataTable UI | Active / DESIGN-agent mockup reviewed; implementation plan recorded for shared numeric alignment, decimal precision investigation, compact notes marker, unit subheaders, status-chip polish, density/shell restyle, toolbar/footer, and browser route matrix | `features/data-table-ui/STATUS.md` |
...
| Table-Write-Architecture Unification | Active / Phases 1–2 (backend) complete — shared `write_spine.py` (all draft-write surfaces) + heat-pumps folded onto the registered contract with a generic `dependent_links` cross-table cascade; bespoke heat-pump write service deleted, old PATCH endpoint kept as a thin shim; `make ci` green (BE 1111, FE 1900). Phase 3 (frontend rewire + old-endpoint removal) ready | `refactor/table-write-architecture-unification/STATUS.md` |

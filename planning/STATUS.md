...
| Table-Write-Architecture Unification | Active / Phase 1 (shared backend write spine) implemented — `replace_table_slice`, schema-mutation, aperture- and envelope-command surfaces converted onto `write_spine.py`, `ty` clean, backend suite 1107 passed; Phase 2 (heat-pumps backend) ready, Phase 3 (frontend rewire) pending | `refactor/table-write-architecture-unification/STATUS.md` |

...
| MCP Write Loop | Active / Phases 1–4 implemented and verified on `codex/mcp-write-loop`: draft save/discard, generic `replace_table`, `preview_replace_table`, `save_draft_as`, version metadata patch, `diff_versions`, canonical MCP docs, drift guard, smoke hardening, and stale contract reconciliation; final archive/closeout next | `features/mcp-write-loop/STATUS.md` |

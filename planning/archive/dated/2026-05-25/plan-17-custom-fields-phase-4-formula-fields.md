...
STATUS: **All sub-phases shipped — P4.0 → P4.10. 2026-05-25.**
        Backend half (P4.0 → P4.6), backend P4.10 acceptance, the
        P4.7 TS port, P4.8 popover + palette + computed-cell,
        P4.9 grid wiring, and the P4.10 frontend acceptance +
        e2e + a11y notes are all in. Open decisions D22–D26
...
RELATED:
  ...
  - planning/archive/dated/2026-05-24/adr-custom-fields-phase-2-errors.md
...
- Full plan-13 §5 Phase 4 exit-criteria walkthrough against a
  ...
  `planning/archive/dated/2026-05-25/screenshots/plan-17-p4-10/`.
...
`planning/archive/dated/2026-05-25/plan-17-a11y-notes.md`. Any blocking
...
| P4.8 | **shipped** | `<FormulaEditorPopover>` (`frontend/src/shared/ui/data-table/components/FormulaEditorPopover.tsx`), `<FormulaFieldPalette>` and `<ComputedCell>` bodies, focused-row live preview wired through `parse` + `resolveRefs` + `evaluate`, display-name re-render from stored AST on open via `rebuildSourceFromStoredAst`. Covered by `frontend/src/shared/ui/data-table/__tests__/FormulaEditorPopover.test.tsx` (9 cases) — focused-row hint, live preview, parse-error gate, self-cycle gate, missing-ref gate, palette insertion at cursor, self-exclusion from palette, happy-path submit, server-error surface + stay-open. Marked shipped retroactively on 2026-05-25 — the implementation landed alongside P4.7 but was not checked off in the doc. |
| P4.9 | **shipped** | `AddFieldPopover` `formula` pill enabled (atomic add-with-config — formula source / AST / deps go into the existing `AddFieldMutation.after.config` slot, no two-round-trip flow); `HeaderContextMenu` `Edit formula…` item gated on `field_type === "formula"`; `handleDuplicateCustomField` formula-guard removed; `buildSetFormulaMutation` added to `customFieldMutations.ts` with `SOURCE_LENGTH_MAX` length cap; grid wiring in `EquipmentTab` exposes `handleEditCustomFieldFormula` + `formulaFieldRegistry` + `getFormulaRowValues` + `rowsComputed` to `RoomsTable`. Computed-cell render path goes through `<ComputedCell>` reading `rows_computed[rowId][cf_id]`. Covered by `frontend/src/features/equipment/__tests__/RoomsTable.formulaField.test.tsx` (computed scalar render, structured-error `#ERROR` render, missing-overlay empty-cell render). Marked shipped retroactively on 2026-05-25. |
| P4.10 | **shipped** | Backend acceptance: 5 tests in `backend/tests/test_project_document_custom_fields_phase_4.py` (formula adds + slice/download overlay parity, missing-ref rejection, self-cycle rejection, type-change-to-formula rejection, and stored-AST identity). Frontend acceptance: `frontend/src/features/equipment/__tests__/RoomsTable.customFieldsPhase4.test.tsx` exercises the full lifecycle through the rendered UI (add formula via popover → edit → duplicate → delete a referenced field → recover by editing) plus a viewer-mode read-only assertion. Playwright e2e walkthrough at `frontend/tests/e2e/custom-fields-phase-4.spec.ts` with screenshots under `planning/archive/dated/2026-05-25/screenshots/plan-17-p4-10/`. A11y findings recorded in `planning/archive/dated/2026-05-25/plan-17-a11y-notes.md`. |

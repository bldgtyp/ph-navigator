...
| **4** — ASHRAE set + selector | ✅ **complete** | Routing decided (private object store, loader-only). Store, loader, widened literal, typed 409 and tests landed 2026-07-26; values published to `ph-navigator-prod` + local MinIO and the `ThermalStandardSelect` / `set_thermal_standard` / `thermal-standards` selector shipped 2026-07-28. **Amended 2026-07-28:** the shipped selector snapped back to the old standard until the project was saved — the command's response slice does not carry the active standard, and the broad thermal invalidation misses the sibling `thermal-standards` key. `useEnvelopeCommandMutation` now writes the new standard into that query's cache. Found in production; the phase's test asserted the POST, not the rendered value. |

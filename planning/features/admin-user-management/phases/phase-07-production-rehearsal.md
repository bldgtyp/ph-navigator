...
SCOPE: Production rehearsal, account lifecycle runbook, and rollout unblock.
RELATED:
  - ../README.md
  - ../STATUS.md
...
# Phase 07 - Production Rehearsal / Rollout Unblock

## Goal

Prove the feature in the environment shape that production rollout will use and
hand `v2-production-rollout` a clean unblock checklist.
...
2. Rehearse on staging or prod-onrender URL before DNS cutover:
   ...
   - grant/revoke Admin;
   - inspect audit rows.
3. Verify cookie/CSRF behavior on the actual deployment origin shape.
4. Verify rate limiting cannot lock out legitimate first use but blocks abuse.
5. Update `planning/features/v2-production-rollout/STATUS.md` when unblocked.
6. Run closeout:
   - simplify skill;
   - docs-pass skill;
   - `make format`;
...
- `make ci` green.
- Production rollout docs reference this feature as complete/unblocked.

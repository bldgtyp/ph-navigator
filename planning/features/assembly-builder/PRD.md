...
| 2026-05-27 | Phase 6 | Attachment row lookup cannot assume every registered table is a plain list. Pumps already stores rows in a `{field_defs, rows}` envelope, while Assembly Builder exposes segments as a flattened virtual table. Generic attachment attach/detach must resolve both shapes through the field registry boundary. | Confirms §5.8 and §13.2: attachment tables are adapter/read-write surfaces, not the canvas domain model. | Keep future attachment-backed table contracts behind shared row-resolution helpers and include envelope-table fixtures in attachment tests. |
| 2026-05-27 | Phase 6 | Evidence surfaces with many attachment cells should resolve visible asset URLs once per view and pass the shared URL map into cells. Letting each cell call bulk-url independently creates an N+1 request pattern on Specifications and the future Site Photos view. | Confirms §13.1: flattened attachment tables are workflow adapters, while shared attachment primitives own preview/download URL resolution. | Keep datasheet/site-photo configs in the shared assets module and batch URL resolution for future attachment-heavy views. |
| 2026-05-27 | Phase 7 | Material catalog refresh needs its own project-material drift report instead of reusing the window-type refresh module directly. The state vocabulary is shared, but material refresh is a semantic envelope command because it must write project-owned material fields and draft etags. | Confirms §7.13: catalog changes never mutate project values until an explicit per-material refresh command is applied. | Consider extracting a small shared drift-comparison helper in Phase 8 after both Windows and Assembly Builder refresh paths stabilize. |

...
DATE: 2026-07-28
TIME: 12:05 EDT
STATUS: Active — D-1 confirmed by Ed; Q-1/Q-2 open, neither blocks Phase 1
...
| --- | --- | --- |
| **A. Private git repo → CI → R2** | `phn-data` holds reviewed, versioned JSON; its Actions validate + publish to R2; app keeps reading only R2 | ✅ **chosen (Ed, 2026-07-28).** History, PR review, schema gates, automated publish, rollback-by-revert — and zero change to the app's runtime posture. Private repos are free; R2 already exists. |
...
blast radius of a compromised `phn-data` token is "bad data published" — which
...
### D-3. The publisher lives in `phn-data`, standalone ✅

`tools/publish.py` is boto3 + jsonschema with no PHN import. Rationale:
- `phn-data` CI stays self-contained (no PHN checkout, no uv env, fast).
...
### D-6. Same bucket, `datasets/` prefix; write token only in `phn-data` ✅
...
`tools/publish.py` against MinIO from a local `phn-data` checkout. Agents and
...
the build into `phn-data` or committing built artifacts. Defer to when a new
...
| E-11 | Token leak from `phn-data` CI | Bucket-scoped; rotate in Cloudflare + repo secret. Damage bounded to bad publishes, which the manifest history makes enumerable and revertible. |
| E-12 | `phn-data` accidentally made public | Process guard only (README warning, repo description, private-forever policy); no technical guard exists. The values would then be a license violation to distribute — same posture as the Dropbox masters today, with better auditability. |
...
| # | Question | State |
| --- | --- | --- |
| Q-1 | **Repo name.** `bldgtyp/phn-data`? (Alternatives: `phn-datasets`, `ph-data`.) | Ed's call at Phase 1 kickoff; cosmetic, nothing downstream depends on it. |
